...
import imgMan from '../assets/man/Naveen Profession1.png';
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uImageResolution: { value: new THREE.Vector2(1920, 1080) }, // Spiderman
      uImageResolution2: { value: new THREE.Vector2(1920, 1080) }, // Man
      uOffset2: { value: new THREE.Vector2(0.0, -0.06) }, // Shift man photo up relative to spiderman
      uScale2: { value: 0.82 }, // Zoom out man photo to match spiderman head size
      uGlobalOffset: { value: new THREE.Vector2(0.0, 0.12) } // Shift BOTH images down to show full faces
    };
        ...
        >
          ...
          <div className="flex-1 max-w-lg lg:max-w-xl text-left">
            ...
            <h1 className="text-2xl md:text-3xl lg:text-[1rem] xl:text-[3.5rem] font-bold tracking-tighter drop-shadow-2xl leading-[1.05] font-sans">
              Frontend<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-200 to-gray-500 font-serif italic font-light pr-2">Developer</span> &<br />
              React Learner
            </h1>
          </div>
          ...
          <div className=" flex-1 max-w-md text-left md:text-right flex flex-col md:items-end">
            <p className="w-110 text-lg md:text-xl text-gray-300 drop-shadow-xl font-light tracking-wide leading-relaxed mb-8">
              I build responsive web projects and problem-solving applications, focusing on clean UI, logical functionality, and exceptional user experience.
            </p>
            
            <a href="/Naveen_resume.pdf" download="Naveen_resume.pdf" className="pointer-events-auto px-8 py-4 rounded-full border border-white/30 text-white text-sm tracking-[0.2em] uppercase font-medium hover:bg-white hover:text-black hover:border-white transition-all duration-500 backdrop-blur-sm shadow-xl inline-block">
              Download Resume
            </a>
          </div>
